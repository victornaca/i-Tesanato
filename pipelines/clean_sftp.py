...
def add_metadata_columns_sftp(df):
    destination_directory = "datalake/clean-data/csv-price-search"
    folder_name = "price"
    os.makedirs(destination_directory, exist_ok=True)
    
    df['load_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    df['source'] = f"sftp/{folder_name}"
...
def load_parquet_files_sftp():
    source_directory =  "datalake/raw-data/csv-price-search"
    folder_name = "price"
